...
import '../cubit/sectors_cubit.dart';

class SectorsManagementScreen extends StatelessWidget {
  const SectorsManagementScreen({super.key});

  @override
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (_) => sl<SectorsCubit>()..load(),
      child: const _SectorsManagementView(),
    );
  }
}

class _SectorsManagementView extends StatefulWidget {
  const _SectorsManagementView();

  @override
  State<_SectorsManagementView> createState() => _SectorsManagementViewState();
}

class _SectorsManagementViewState extends State<_SectorsManagementView> {
  ...
  void _onSearchChanged(String value) {
    ...
    _searchDebounce = Timer(const Duration(milliseconds: 400), () {
      context.read<SectorsCubit>().search(value);
    });
  }
  ...
  Future<void> _openCreateSheet() async {
    final result = await MasterDataFormSheet.show(
      context,
      addTitleKey: AppString.addSector,
      editTitleKey: AppString.editSector,
    );
    if (result == null || !mounted) return;

    final success = await context.read<SectorsCubit>().createSector(
          SectorWriteRequest(
            title: result.title,
  ...
  Future<void> _openEditSheet(Sector sector) async {
    final result = await MasterDataFormSheet.show(
      context,
      addTitleKey: AppString.addSector,
      editTitleKey: AppString.editSector,
      initialTitle: sector.title,
      initialDescription: sector.description,
    );
    if (result == null || !mounted) return;

    final success = await context.read<SectorsCubit>().updateSector(
          id: sector.id,
          request: SectorWriteRequest(
            title: result.title,
            description: result.description,
          ),
        );
  ...
  Future<void> _confirmDelete(Sector sector) async {
    final confirmed = await showDialog<bool>(
      context: context,
      builder: (_) => DeleteConfirmationDialog(
        titleKey: AppString.confirmDelete,
        messageKey: AppString.confirmDeleteSector,
      ),
    );

    if (confirmed != true || !mounted) return;

    final success = await context.read<SectorsCubit>().deleteSector(sector.id);
    if (success && mounted) {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        centerTitle: true,
        title: Text(
          AppString.sectorsManagement.tr(),
          style: TextStyle(
      ...
      body: BlocBuilder<SectorsCubit, SectorsState>(
        builder: (context, state) {
              ...
              child: Padding(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    FilledButton(
                      onPressed: () => context.read<SectorsCubit>().load(),
          ...
          return RefreshIndicator(
            color: colors.kPrimaryColor,
            onRefresh: () => context.read<SectorsCubit>().refresh(),
            child: ListView(
              ...
              children: [
                ...
                MasterDataManagementTable(
                  ...
                  onEdit: (item) => _openEditSheet(
                    state.items.firstWhere((sector) => sector.id == item.id),
                  ),
                ...
                if (state.totalCount > 0)
                  Container(
                    ...
                    child: TablePaginationWidget(
                      ...
                      onPageChanged: context.read<SectorsCubit>().changePage,
                      onPageSizeChanged:
                          context.read<SectorsCubit>().changePageSize,
                    ),
                  ),
              ],
            ),
